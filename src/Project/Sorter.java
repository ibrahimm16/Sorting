...
public class Sorter {
    ...
    boolean running = true, begun = false;
    double delta;
    long now, lastTime;
    ...
    class SortingManager implements Runnable {
        ...
        void event(SortingAlgorithm timedSort, SortingAlgorithm renderedSort) {
            LinkedList<Integer> unsortedList = new LinkedList<>(timedSort.getTimedList());
    ...
    class EventManager implements KeyListener {
        ...
        public void keyPressed(KeyEvent e) {
            if (e.getKeyCode() == KeyEvent.VK_ENTER && !begun) {
                while (!begun) {
                    try {
                        Thread sortingThread = new Thread(sortingManager);
                        sortingThread.start();
                        begun = true;
